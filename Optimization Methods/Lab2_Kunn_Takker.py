...
def Kunn_Takker():
    mat = np.column_stack((A, c))
    mat = np.row_stack((mat, np.append(c, 0)))
    print ("Matrix A*: \n", mat)
    x_min = np.linalg.solve(mat, -np.append(b, d))
    print ("x_min =", x_min)
    print ("f(x_min) =", f(x_min[:6]))
    print ("x_0 =", analytical_solution())
    print ("f(x_0) =", f(analytical_solution()))


n = 6
d = 0
c = np.array([1, 2, 1, 2, 5, 1])
